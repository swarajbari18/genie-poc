...
import { verifyCallback, downloadSignedFile, } from '../services/boldsignClient.js';
 ...
 */
function mapBoldsignSignerStatus(raw) {
    if (!raw)
        return null;
...
function getBoldsignEventType(event) {
    return event?.event?.eventType ?? event?.context?.eventType;
}
function getBoldsignDocumentId(event) {
    return (event?.data?.documentId ??
        event?.data?.document?.documentId ??
        event?.documentId ??
        event?.document?.documentId);
}
function getBoldsignSignerDetails(event) {
    const details = event?.data?.signerDetails ?? event?.data?.document?.signerDetails;
    return Array.isArray(details) ? details : [];
}
async function processBoldsignEvent(contractId, ownerId, event) {
    const eventType = getBoldsignEventType(event);
    const documentId = getBoldsignDocumentId(event);
    ...
    for (const sd of signerDetails) {
        ...
        await db
            ...
            .set(patch)
            .where(and(eq(contractSigners.signatureRequestId, documentId), sql `lower(${contractSigners.signerEmail}) = lower(${email})`));
    }
    // Find the "affected" signer for narrative thread messages (per-signer events).
    function findSignerByStatus(target) {
    ...
    switch (eventType) {
        case 'Sent': {
            ...
            const pending = (contractRow?.pendingSigners ?? []);
            if (pending.length === 0) {
                ...
                await db.transaction(async (tx) => {
                    for (const ps of pending) {
                        const initialStatus = ps.order === Math.min(...pending.map((p) => p.order)) ? 'sent' : 'pending';
                        await tx
        ...
        case 'Viewed': {
            ...
            if (who)
                threadMessage = `${who.signerName ?? who.signerEmail} viewed the contract`;
            break;
        }
        case 'Signed': {
            ...
            if (who)
                threadMessage = `${who.signerName ?? who.signerEmail} signed`;
            break;
        }
        case 'Declined': {
    ...
    if (contractStatusUpdate) {
        await db
            .update(contracts)
            .set({ status: contractStatusUpdate, updatedAt: new Date() })
            .where(eq(contracts.id, contractId));
    }
    if (threadMessage) {
...
async function processInbound(inboundId, payload) {
    ...
    const rawInReplyTo = payload.Headers
    ...
    if (inReplyTo) {
        ...
        const [matchedThread] = await db
            ...
            .where(and(eq(contractThreads.postmarkMessageId, inReplyTo), eq(contracts.userId, serviceEmail.userId)))
            .limit(1);
    ...
    const rfcMessageId = payload.Headers
        ?.find(h => h.Name?.toLowerCase() === 'message-id')?.Value;
    const inboundMessageIdNormalized = normalizeMessageId(rfcMessageId) ??
