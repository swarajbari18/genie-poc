 ...
 */
export async function resolveDeliveryAddress(identityEmail) {
    const domain = process.env.MAIL_DOMAIN;
    if (identityEmail.toLowerCase().endsWith(`@${domain}`)) {
        return identityEmail;
    }
    // Look up a Genie user whose sign-in email matches.
    const rows = await db.execute(sql `SELECT id FROM "user" WHERE lower(email) = ${identityEmail.toLowerCase()} LIMIT 1`);
