...
export function contractTextToHtml(title: string, text: string): string {
  const escaped = text
    .replace(/&/g, '&amp;')
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;')
...
export function buildHtmlShell(title: string, bodyHtml: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
export async function generateContractPdf(opts: { title: string; text: string }): Promise<Buffer> {
