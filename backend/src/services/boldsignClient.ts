...
export interface CreatePrepareResult {
  documentId: string
  sendUrl: string
}
 ...
 */
export async function createEmbeddedPrepareRequest({
  title,
  message,
  pdfBuffer,
  pdfFilename,
  ...
  try {
    const boldsignSigners = signers.map((s) => {
      ...
      if (s.order != null) ds.signerOrder = s.order
      return ds
    })
    ...
    req.files = [fs.createReadStream(tmpPath) as any]
    ...
    if (redirectUrl) req.redirectUrl = redirectUrl
    ...
    if (!documentId || !sendUrl) {
      throw new Error(
        `BoldSign createEmbeddedRequestUrlDocument returned incomplete response (documentId=${documentId}, sendUrl=${!!sendUrl})`,
      )
    }
...
export async function getEmbeddedSignUrl(
  ...
  const res = await documentApi.getEmbeddedSignLink(
    documentId,
    signerEmail,
    undefined,
    undefined,
  ...
  if (!signLink) {
    throw new Error(
      `Failed to get embedded sign URL for document=${documentId} signer=${signerEmail}`,
    )
  }
  return signLink
}
 ...
 */
export function verifyCallback(rawBody: string, signatureHeader: string | null): boolean {
  if (!WEBHOOK_SECRET) {
...
export async function downloadSignedFile(documentId: string): Promise<Buffer> {
