...
function applyTool(
...
): { success: boolean; newText: string; message: string } {
  const isSectionHeading = (line: string) =>
    /^\d+\./.test(line.trim()) || /^[A-Z][A-Z\s.,()-]{4,}$/.test(line.trim())

  if (toolName === 'replace_text') {
    const { find, replace } = args
    if (!text.includes(find)) return { success: false, newText: text, message: 'Text not found — try a shorter verbatim quote from the contract' }
  ...
  if (toolName === 'replace_section') {
    ...
    if (startIdx === -1) return { success: false, newText: text, message: 'Section not found' }
...
- Never rewrite the full contract in your text reply — use tools for edits.`,
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const toolResults = calls.map((call) => {
        const result = applyTool(proposedText, call.name, call.args as Record<string, string>)
        if (result.success) proposedText = result.newText
