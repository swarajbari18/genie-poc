...
const contractsRouter = new Hono<HonoEnv>()
...
contractsRouter.post('/:id/send', requireAuth, async (c) => {
  ...
  if (recipients.length === 0) return c.json({ error: 'At least one recipient required' }, 400)
  if (recipients.length > 10) return c.json({ error: 'Maximum 10 recipients' }, 400)
  for (const r of recipients) {
    if (!r.email?.trim()) return c.json({ error: 'Each recipient must have an email' }, 400)
  }
...
contractsRouter.post('/:id/reply', requireAuth, async (c) => {
  ...
  let attachmentMeta:
    | { filename: string; contentType: string; storageKey: string; sizeBytes: number }
    | undefined
  ...
  if (!isOwner) {
    const signerRow = await db.query.contractSigners.findFirst({
      where: and(
        ...
        eq(contractSigners.genieUserId, user.id),
        inArray(contractSigners.status, ['pending', 'sent', 'viewed', 'signed']),
      ),
    })
...
contractsRouter.post('/:id/send-for-signature', requireAuth, async (c) => {
  ...
  for (const [idx, s] of signers.entries()) {
    ...
    let genieUserId: string | null = null
    ...
    if (byIdentity.length > 0) {
      const gUser = byIdentity[0] as { id: string; email: string }
      ...
      if (byService) {
        ...
        if (ownerUser.length > 0) {
          const gUser = ownerUser[0] as { id: string; email: string }
          genieUserId = gUser.id
    ...
    if (sendToEmail.toLowerCase().endsWith(`@${MAIL_DOMAIN}`)) {
      return c.json(
        { error: `Cannot send signing link to service address: ${sendToEmail}` },
        400,
      )
    }
  ...
  const boldsignSigners: SignerOptions[] = resolvedSigners.map((s) => ({
    name: s.name,
...
contractsRouter.get('/:id/sign-url', requireAuth, async (c) => {
  ...
  const signerRow = await db.query.contractSigners.findFirst({
    where: and(
      eq(contractSigners.contractId, contractId),
      eq(contractSigners.genieUserId, user.id),
      inArray(contractSigners.status, ['pending', 'sent', 'viewed']),
    ),
  })
  if (!signerRow) return c.json({ error: 'No pending signature found for this user' }, 404)
...
contractsRouter.patch('/:id/html', requireAuth, async (c) => {
  ...
  if (!contract) return c.json({ error: 'Not found' }, 404)
  if (contract.status !== 'draft') return c.json({ error: 'Only draft contracts can be edited' }, 409)

  const v1 = await db.query.contractVersions.findFirst({
    where: and(
      eq(contractVersions.contractId, contractId),
      eq(contractVersions.versionNumber, 1),
    ),
  })
