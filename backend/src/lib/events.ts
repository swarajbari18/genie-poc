 ...
 */
export async function bootEventsListener(): Promise<void> {
  if (booted) return
  booted = true
  ...
  await sql.listen(
    ...
    (payload: string) => {
      try {
        const parsed = JSON.parse(payload) as UserUpdatePayload
        log.info('events', 'NOTIFY received', parsed as unknown as Record<string, unknown>)
...
export async function notifyUser(
  userId: string,
  data: { contractId: string; status: string },
): Promise<void> {
  const payload: UserUpdatePayload = { userId, contractId: data.contractId, status: data.status }
  ...
  try {
    ...
    log.info('events', 'NOTIFY sent', payload as unknown as Record<string, unknown>)
  } catch (err: any) {
